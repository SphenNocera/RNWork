...
def get_lines_in_image(image):
    ...
    # Image.fromarray(all_lines).show()
    all_lines = cv.dilate(all_lines, cv.getStructuringElement(cv.MORPH_RECT, (3, 3)))
    # Image.fromarray(all_lines).show()

    final = cv.inpaint(image, all_lines, 3, cv.INPAINT_TELEA)
